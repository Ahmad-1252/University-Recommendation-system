...
Tests each university by:
1. Starting from the last university (Athens) and working up to Oxford
2. Testing ALL levels (undergraduate, graduate, phd) for each university
3. Scraping 10 programs total, distributed across all available levels
4. Generating a detailed report with success/failure status per level

Usage:
    python test_universities_bottom_up.py                      # Test all universities bottom-up
    python test_universities_bottom_up.py --limit 5            # Test first 5 (from bottom)
    python test_universities_bottom_up.py --dry-run            # Only discover links, don't scrape
    python test_universities_bottom_up.py --output report.json # Save report to file
"""
...
async def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python test_universities_bottom_up.py                      # Test all universities
    python test_universities_bottom_up.py --limit 3            # Test first 3 from bottom
    python test_universities_bottom_up.py --skip 5             # Skip first 5 universities  
    python test_universities_bottom_up.py --programs 15        # Scrape 15 programs per university
    python test_universities_bottom_up.py --dry-run            # Only discover links
    python test_universities_bottom_up.py --output report.json # Save report
        """
    )
...
#!/usr/bin/env python
"""
Deprecated: this script has been moved to `tests/integration/test_universities_bottom_up.py`.
Run the new script or use `pytest` to execute integration tests.
"""
from pathlib import Path
script = Path(__file__).resolve().parent / 'tests' / 'integration' / 'test_universities_bottom_up.py'
print(f"This test script has been moved to: {script}\nRun it using: python {script} or pytest tests/integration")
