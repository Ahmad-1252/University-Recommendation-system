...
async def scrape_all_programs(
    ...
    if university:
        universities = {
            k: v for k, v in universities.items() if university.lower() in k.lower()
        }
    ...
    console.print(
        f"[cyan]Program Categories: {', '.join(PROGRAM_CATEGORIES.keys())}[/cyan]"
    )
    ...
    for uni_name, directories in universities.items():
        ...
        if degree_level:
            ...
            levels_to_scrape = level_map.get(
                degree_level.lower(), [degree_level.lower()]
            )
                ...
                all_urls.append(
                    {
                        ...
                        "program_url_pattern": directories.get(
                            "program_url_pattern", ""
                        ),
    ...
    for item in all_urls:
        try:
            ...
            console.print(
                f"[green]✓ Found {programs_found} programs, saved {programs_saved}[/green]"
            )
...
Examples:
  ...
  python run_scraper.py --single-url              # Legacy mode: scrape 1 program per university
    ...
    parser.add_argument(
        "--urls", nargs="*", help="Specific URLs to scrape (enables single-url mode)"
    )
    parser.add_argument("--concurrent", type=int, default=3, help="Concurrent requests")
    parser.add_argument(
        "--limit", type=int, help="Limit number of universities to process"
    )
    ...
    parser.add_argument(
        "--university", type=str, help="Filter by university name (partial match)"
    )
    parser.add_argument(
        "--level",
        type=str,
        choices=["undergraduate", "graduate", "masters", "phd", "all"],
        default="all",
    ...
    if args.single_url or args.urls:
        if args.urls:
            ...
            console.print(
                f"[cyan]Using {len(urls)} pre-configured university URLs (single-url mode)[/cyan]"
            )
    ...
    console.print("[cyan]🚀 Multi-program discovery mode (default)[/cyan]")
