...
class UniversityDataExporter:
    ...
    def export_to_csv(self, programs: List[Dict[str, Any]], filename: str = None) -> bool:
    ...
    def export_to_excel(self, programs: List[Dict[str, Any]], filename: str = None) -> bool:
        """Export programs to Excel file with formatting"""
        try:
            if not OPENPYXL_AVAILABLE:
                logger.warning("[WARNING] openpyxl not installed. Install with: pip install openpyxl")
                return False
    ...
    def export_to_json(self, programs: List[Dict[str, Any]], filename: str = None) -> bool:
        ...
        try:
            ...
            for program in programs:
                ...
                if 'extracted_at' in program_copy and isinstance(program_copy['extracted_at'], datetime):
                    program_copy['extracted_at'] = program_copy['extracted_at'].isoformat()
    ...
    def export_to_pandas_excel(self, programs: List[Dict[str, Any]], filename: str = None) -> bool:
        """Export programs to Excel using pandas (requires pandas)"""
        try:
            if not PANDAS_AVAILABLE:
                logger.warning("[WARNING] pandas not installed. Use export_to_excel() instead.")
                return False

            if filename is None:
                filename = f"programs_pandas_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            ...
            preferred_columns = [
                'university_name', 'program_name', 'degree_type', 'duration',
                'tuition_fees', 'admission_requirements', 'language_requirements',
                'application_deadline', 'program_url', 'country', 'city', 'ranking',
                'description', 'confidence_score', 'extracted_at'
            ]
            ...
            logger.info(f"[OK] Exported {len(programs)} programs to Excel (pandas): {filepath}")
            return True
        ...
        try:
            ...
            for program in programs:
                try:
                    ...
                    logger.error(f"[ERROR] Failed to save program {program.get('program_name', 'Unknown')}: {e}")
                    continue
