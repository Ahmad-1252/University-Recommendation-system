...
def search(ctx, query, university, country, degree, limit):
    ...
    try:
        programs = repository.search(
            query=query, country=country, degree_type=degree, limit=limit
        )
...
def scrape(ctx, urls, file, concurrent):
    ...
    if not urls:
        console.print(
            "[red]No URLs provided. Use --file or provide URLs as arguments[/red]"
        )
        return

    console.print(
        f"[green]Starting scrape of {len(urls)} URLs (concurrent: {concurrent})[/green]"
    )
    ...
    try:
        ...
        console.print(
            f"[green]Scraping completed: {successful} successful, {failed} failed[/green]"
        )
...
@click.option(
    ...
    "-f",
    type=click.Path(exists=True),
    help="File containing URLs to validate",
)
@click.pass_context
def validate(ctx, urls, file):
    ...
    try:
        ...
        console.print(
            f"[green]Validation completed: {valid_count} valid, {invalid_count} invalid[/green]"
        )
...
def export(ctx, output, formats):
    ...
    try:
        ...
        console.print(
            f"[green]Exporting {len(programs)} programs to {output_path}[/green]"
        )
