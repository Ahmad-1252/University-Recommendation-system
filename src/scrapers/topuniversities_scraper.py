...
class TopUniversitiesScraper(BaseScraper):
    ...
    async def scrape_program_data(
        ...
        raise NotImplementedError(
            "TopUniversities scraper is designed for rankings data, not programs."
        )
    ...
    async def scrape_world_rankings(
        self, year: Optional[int] = None, max_results: int = 500
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _scrape_rankings_page(
        self, year: Optional[int], max_results: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _scroll_to_load_content(self, page: Page, target_count: int):
        ...
        while attempts < max_attempts:
            ...
            current_count = 0
            for selector in [
                self.selectors["ranking_item"],
                self.selectors["ranking_item_card"],
                "a.uni-link",
            ]:
                try:
        ...
        try:
            ...
            if not location_elem:
                location_elem = item.select_one(
                    ".location, .country, [class*='location']"
                )
    ...
    def _parse_location(
        ...
        if len(parts) == 3:
            campus, city, country = parts
            return campus, country, city
        elif len(parts) == 2:
            city, country = parts
            return country, city
        elif len(parts) == 1:
    ...
    async def scrape_subject_rankings(
        self, subject: str, year: Optional[int] = None, max_results: int = 200
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            if not self.browser:
                async with self:
                    return await self._scrape_subject_rankings_page(
    ...
    async def _scrape_subject_rankings_page(
        self, subject: str, year: Optional[int], max_results: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _scrape_profile_page(self, university_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            name_elem = soup.select_one("div.univ-logo-n-name h1")
            if name_elem:
                profile_data["university_name"] = name_elem.get_text(strip=True)

            # Extract overview/description
            desc_elem = soup.select_all(
                "div#p2-overview div.textsection.abt-overview-read div.details"
            )[-1]
            if desc_elem:
            ...
            ranking_sections = soup.select(
                ".ranking-score, [data-testid='ranking-score']"
            )
            for section in ranking_sections:
                ...
                if rank_type and rank_value:
                    rankings[rank_type.get_text(strip=True)] = rank_value.get_text(
                        strip=True
                    )
            ...
            website_elem = soup.select_one("div.univ-logo-n-name h1 a")
            if website_elem:
                profile_data["website"] = website_elem.get("href")

            # Extract location
            location_elem = soup.select_one("div.uni-location.text-white h2")
            if location_elem:
                location_text = location_elem.get_text(strip=True)
                campus, country, city = self._parse_location(location_text)
                profile_data["campus"] = campus
                profile_data["country"] = country
                profile_data["city"] = city

            # Extract student numbers
            students_elem = soup.select_one(
                ".student-count, [data-testid='student-count']"
            )
            if students_elem:
                ...
                if match:
                    try:
                        profile_data["total_students"] = int(
            ...
            await page.context.close()
            logger.info(
                f"Successfully scraped profile for {profile_data.get('university_name', 'Unknown')}"
            )
        ...
        try:
            ...
            if await search_input.count() > 0:
                ...
                result_link = page.locator(
                    f"a[href*='/universities/']:has-text('{university_name[:20]}')"
                )
    ...
    async def _scrape_programs_api(
        self, max_programs: int = 100, study_level: str = None
    ) -> list:
            ...
            try:
                ...
                resp = await self._http_client.get(
                    ...
                    headers={
                        ...
                        "Referer": f"{self.base_url}/programs",
                    },
                    ...
                    results.append(
                        {
                            ...
                            "university_name": item.get("uni_name"),
                            "country": country[0]
                            ...
                            "url": f"{self.base_url}{item.get('program_url')}"
    ...
    async def scrape_programs_with_details(
        self, max_programs: int = 100, study_level: str = None, on_progress=None
    ) -> list:
            ...
            async with semaphore:
                try:
                    ...
                    response = await self._http_client.get(
                        ...
                        timeout=self.rate_limiting.get("timeout_seconds", 30),
                    ...
                    logger.error(
                        f"Deep scrape failed for {url}: {e}\n{traceback.format_exc()}"
                    )
                    return program

        enriched = await asyncio.gather(
            *[enrich_one(p) for p in discovery], return_exceptions=True
        )
        final_results = []
        for r in enriched:
    ...
    def _extract_page_fields(self, soup, url: str) -> dict:
        ...
        for box in soup.select(
            ".badge-description, .program-quick-details-v2 .detail-box, .program-quick-details .detail-box"
        ):
            ...
            if label_elem and val_elem:
                ...
                if "duration" in label:
                    ...
                    detail["tuition_fee_summary"] = value
                elif "degree" in label:
                    detail["degree_type"] = value
                elif "level" in label:
                    detail["study_level"] = value
        ...
        for fee_div in soup.select(".fee-block, .tuition-fees, .tuition-card, div"):
            ...
            if "domestic" in text and not detail["fee_domestic"]:
                ...
                if m:
                    detail["fee_domestic"] = m.group(1)
            if "international" in text and not detail["fee_international"]:
                m = re.search(
                    r"international.*?(?:£|\$|€|sgd|usd|gbp)\s*([0-9,]+)", text
                )
                if m:
                    detail["fee_international"] = m.group(1)
