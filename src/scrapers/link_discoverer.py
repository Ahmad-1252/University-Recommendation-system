...
from core.constants import PROGRAM_CATEGORIES, DEGREE_LEVELS
...
class LinkDiscoverer:
    ...
    def __init__(self):
        ...
        self.program_keywords.update({
            "program", "programme", "course", "degree", "study", "major",
            "curriculum", "admission", "apply", "enrol", "enroll"
        })

        # URL patterns to avoid
        self.exclude_patterns = {
            "facebook.com", "twitter.com", "linkedin.com", "instagram.com",
            "youtube.com", "calendar", "events", "news", "blog", 
            "staff", "faculty-profile", "alumni", "login", "register",
            "privacy", "terms", "cookie", "accessibility", "sitemap",
            "careers", "jobs", "vacancies", "#", "javascript:", "mailto:",
            "tel:", ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".zip"
        }

    async def discover_all_program_links(self, 
                                         directory_url: str,
                                         base_url: str = None,
                                         category_filter: str = None,
                                         program_url_pattern: str = None,
                                         pagination: dict = None) -> List[Dict[str, str]]:
        """
        ...
        if program_url_pattern:
            try:
                ...
                logger.warning(f"Invalid program URL pattern '{program_url_pattern}': {e}")
            ...
            async with AsyncWebCrawler() as crawler:
                ...
                if pagination:
                    ...
                    if hasattr(result, 'links') and result.links:
                        all_result_links = result.links.get('internal', []) + result.links.get('external', [])
                        if all_result_links:
                            discovered_programs = self._extract_programs_from_result_links(
                                all_result_links, base_url, compiled_pattern, category_filter, []
                            )
    ...
    def _extract_programs_from_result_links(self, links: list, base_url: str,
                                            compiled_pattern, category_filter: str,
                                            existing_programs: list) -> List[Dict[str, str]]:
        ...
        for link_info in links:
            ...
            if compiled_pattern:
                if compiled_pattern.search(absolute_url):
                    program_name = text if text and len(text) > 3 else self._extract_program_name_from_url(absolute_url)
                    
                    if program_name:
                        detected_category = self._detect_category(program_name, absolute_url)
                        
                        if category_filter and detected_category.lower() != category_filter.lower():
                            continue
                        
                        degree_level = self._detect_degree_level(program_name, absolute_url)
                        ...
                        if not any(p['program_url'] == absolute_url for p in discovered_programs):
                            if not any(p['program_url'] == absolute_url for p in existing_programs):
    ...
    def _extract_programs_from_soup(self, soup: BeautifulSoup, base_url: str, 
                                     compiled_pattern, category_filter: str,
                                     existing_programs: list) -> List[Dict[str, str]]:
    ...
    def _extract_program_name_from_url(self, url: str) -> str:
        ...
        title_words = []
        for word in words:
            if word.upper() in ['BSC', 'MSC', 'MBA', 'PHD', 'BA', 'MA', 'MENG', 'BENG', 'LLM', 'LLB']:
    ...
    def _is_program_link(self, url: str, link_text: str) -> bool:
        ...
        skip_texts = [
            "home", "back", "next", "previous", "page", "more", "see all",
            "read more", "learn more", "click here", "view all", "contact",
            "enquire", "apply now", "apply online", "download", "print",
            "menu", "skip", "search", "login", "sign in", "subscribe",
            "follow us", "share", "tweet", "facebook", "twitter", "linkedin",
            "instagram", "youtube", "email", "phone", "address", "map",
            "privacy", "terms", "cookies", "accessibility", "sitemap",
            "about us", "about the", "our mission", "our values",
            "staff", "faculty", "alumni", "news", "events", "blog",
            "careers", "jobs", "vacancies", "work with us",
            "support researchers", "engage with us", "recognition",
            "partnerships", "collaboration", "funding", "grants",
            "where do i start", "getting started", "open days", "changes to",
            "departments", "research courses", "taught courses", "how to apply",
            "fees", "scholarships", "visa", "accommodation", "international",
            "student portal", "media information", "newsfeed", "contact"
        ]
        ...
        non_program_url_sections = [
            "/the-eth-zurich/", "/portrait/", "/sustainability/",
            "/working-teaching", "/vocational-education", "/quality-management/",
            "/educational-development/", "/ai-in-education/", "/information-material/",
            "/continuing-education", "/research/", "/news/", "/events/",
            "/open-science/", "/doctorate.", "/utils/", "/footer/",
            "/login/", "/search/", "/contact/", "/media/", "/about/",
            "/staff/", "/alumni/", "/careers/", "/jobs/", "/vacancies/",
            "/student-life/", "/wellbeing/", "/support/", "/finance/",
            "/prospective-students", "/application/", "/beginning-your",
            "/admissions/", "/funding/", "/scholarships/", "/fees/",
            # Section path patterns
            "getting-started", "taught-courses", "research-courses",
            "open-days", "changes-to", "departments", "how-to-apply",
            "fees-and-funding", "international-applicants", "advice-",
            "why-oxford", "sport-arts", "disability", "colleges",
            "entry-requirements", "general-information", "choosing-a",
            "masterprogrammas", "masters-programmes.", "bachelors-programmes.",
            "/d-arch/", "/d-baug/", "/d-biol/", "/d-bsse/", "/d-chab/",
            "/d-eaps/", "/d-gess/", "/d-hest/", "/d-infk/", "/d-itet/",
            "/d-math/", "/d-matl/", "/d-mavt/", "/d-mtec/", "/d-phys/", "/d-usys/"
        ]
        ...
        generic_url_endings = [
            "degree-programmes.html", "degree-programs.html",
            "bachelors-degree-programmes.html", "masters-degree-programmes.html",
            "/programmes.html", "/programs.html", "/courses.html",
            "/bachelors.html", "/masters.html", "/undergraduate.html",
            "/graduate.html", "/postgraduate.html", "/index.html"
        ]
        ...
        eth_program_pattern = re.search(
            r'/degree-programmes?/[^/]+/([a-z0-9-]+)\.html$', 
            url_lower
        )
        if eth_program_pattern:
            ...
            category_slugs = [
                'engineering-sciences', 'natural-sciences-and-mathematics',
                'architecture-and-civil-engineering', 'management-and-social-sciences',
        ...
        degree_prefixes = [
            r'/msc-[a-z]', r'/ma-[a-z]', r'/mba[/-]?', r'/mth-[a-z]', r'/mst-[a-z]',
            r'/bsc-[a-z]', r'/ba-[a-z]', r'/beng-[a-z]', r'/meng-[a-z]',
            r'/phd-[a-z]', r'/dphil-[a-z]', r'/dphil\b', r'/phd\b',
            r'/llm-[a-z]', r'/llb-[a-z]', r'/med-[a-z]', r'/md-[a-z]',
            r'/mph-[a-z]', r'/mph\b', r'/mphil-[a-z]', r'/mres-[a-z]',
            r'/pgcert-[a-z]', r'/pgdip-[a-z]', r'/postgrad-[a-z]',
            r'/mlitt-[a-z]', r'/mmus-[a-z]', r'/march-[a-z]',
            r'/msci-[a-z]', r'/mmath-[a-z]', r'/mchem-[a-z]', r'/mphys-[a-z]',
        ]
        ...
        if len(path_parts) >= 2:
            ...
            generic_endings = [
                "courses", "programmes", "programs", "degrees", "undergraduate", 
                "postgraduate", "graduate", "taught", "research", "admissions",
                "study", "index", "listing", "list", "all", "a-z", "course-listing",
                "masters-programmes", "bachelors-programmes", "phd-programmes",
                "masters", "bachelors", "overview"
            ]
            if last_segment in generic_endings:
                return False
            
            # URL last segment starts with degree code (e.g., msc-advanced-computing)
            degree_codes = ['msc', 'ma', 'mba', 'mth', 'mst', 'bsc', 'ba', 'beng', 
                           'meng', 'phd', 'dphil', 'llm', 'llb', 'med', 'md', 
                           'mph', 'mphil', 'mres', 'pgcert', 'pgdip', 'mlitt',
                           'mmus', 'march', 'msci', 'mmath', 'mchem', 'mphys']
            ...
            degree_directories = ['masters', 'bachelors', 'undergraduate', 'postgraduate', 
                                 'graduate', 'phd', 'doctoral', 'research-degrees', 
                                 'master', 'bachelor', 'master-s', 'bachelor-s']
            ...
            if parent_segment in degree_directories:
                ...
                if '-' in last_segment and len(last_segment) > 5:
                    ...
                    if '?' not in url_lower:
                        non_program_text = ['contact', 'info', 'about', 'apply', 'faq', 
                                           'news', 'events', 'all-programmes', 'overview',
                                           'all-courses', 'course-listing']
            ...
            if len(path_parts) >= 4:
                if 'courses' in path_parts:
                    ...
                    if courses_idx + 2 < len(path_parts):
                        ...
                        if year_or_category.isdigit() and len(path_parts) > courses_idx + 3:
                        ...
                        if program_slug not in generic_endings and '-' in program_slug:
                            ...
                            admin_paths = ['offer-holders', 'application-process', 
                                          'teachers-conference', 'imperial-on-tour',
                                          'information-by-region', 'executive-education']
            ...
            if '-' in last_segment and len(last_segment) > 8 and len(text_lower) > 10:
                ...
                subject_keywords = [
                    "science", "arts", "engineering", "business", "medicine", "law",
                    "education", "computer", "computing", "data", "software", "network",
                    "information", "management", "finance", "accounting", "marketing",
                    "economics", "nursing", "pharmacy", "healthcare", "clinical",
                    "medical", "teaching", "curriculum", "mathematics", "physics",
                    "chemistry", "biology", "psychology", "history", "philosophy",
                    "literature", "linguistics", "sociology", "politics", "international",
                    "environmental", "sustainability", "energy", "architecture",
                    "theology", "archaeology", "anthropology", "geography", "music",
                    "biotechnology", "bioinformatics", "robotics", "quantum", "nuclear",
                    "materials", "mechanical", "electrical", "civil", "cyber", "security"
                ]
                ...
                if any(kw in text_lower for kw in subject_keywords):
                    ...
                    if not any(skip in text_lower for skip in ['department', 'd-', 'faculty']):
                        return True
    ...
    def _detect_degree_level(self, text: str, url: str) -> str:
        ...
        if any(kw in combined for kw in ["phd", "doctor", "doctoral", "dphil", "research degree"]):
            return "PhD"
        if any(kw in combined for kw in ["master", "msc", "ma", "mba", "meng", "mphil", "postgraduate taught"]):
            return "Masters"
        if any(kw in combined for kw in ["bachelor", "bsc", "ba", "beng", "undergraduate"]):
