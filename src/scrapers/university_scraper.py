...
class UniversityScraper(BaseScraper):
        ...
        try:
            ...
            async with AsyncWebCrawler() as crawler:
                ...
                if not result.success:
                    logger.error(f"Crawl failed for {url}: {result.error_message if hasattr(result, 'error_message') else 'Unknown error'}")
                    return None
                ...
                program = await self.content_extractor.extract_program_data(url, html_content)
    ...
    async def scrape_all_programs_from_directory(self,
                                                  directory_url: str,
                                                  base_url: str = None,
                                                  ...
                                                  pagination: dict = None) -> List[UniversityProgram]:
        """
        ...
        for i in range(0, len(urls_to_scrape), concurrent):
            ...
            for url, program in scraped_data.items():
                if program:
                    ...
                    original_info = next((p for p in program_links if p['program_url'] == url), None)
                    if original_info:
                        # Add discovered metadata if not already set
                        if not program.program_category or program.program_category == "Other":
                            program.program_category = original_info.get('category', 'Other')
                        if not program.degree_level or program.degree_level == "Masters":
                            program.degree_level = original_info.get('degree_level', 'Unknown')
        ...
        logger.info(f"Successfully scraped {len(all_programs)} programs from {directory_url}")
        return all_programs

    async def scrape_all_programs_and_save(self,
                                           directory_url: str,
                                           base_url: str = None,
                                           category_filter: str = None,
                                           max_programs: int = 100,
                                           program_url_pattern: str = None,
                                           pagination: dict = None) -> Dict[str, any]:
        """
        ...
        if programs:
            ...
            stats["programs_saved"] = save_results.get("inserted", 0) + save_results.get("updated", 0)
            stats["failed"] = save_results.get("failed", 0)
            
            # Update university with aggregated data from all program pages
            university_names = set(p.university_name for p in programs if p.university_name)
            for uni_name in university_names:
                try:
                    ...
                    logger.warning(f"Failed to update university data for {uni_name}: {e}")
    ...
    async def _extract_and_aggregate_university_data(
        ...
        try:
            ...
            uni_data = await self.content_extractor.extract_university_data(url, html_content)
            ...
            if uni_data:
                ...
                logger.debug(f"Aggregated university data for {university_name} from {url}")
                
        except Exception as e:
            logger.debug(f"Failed to extract university data from {url}: {e}")

    def _merge_university_data(self, aggregated: Dict[str, Any], new_data: Dict[str, Any]) -> None:
        """
        ...
        single_value_fields = [
            "website", "admissions_url", "email", "phone", "founding_year",
            ...
            "libraries_count", "logo_url", "mascot", "latitude", "longitude",
            "campus_type", "campus_size_acres", "state_province"
        ]
    ...
    async def update_university_with_aggregated_data(self, university_name: str) -> bool:
        """
        ...
        logger.info(f"Updating university {university_name} with {len(aggregated_data)} aggregated fields")
