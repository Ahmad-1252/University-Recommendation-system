...
def normalize_degree_level(value: str) -> str:
    ...
    if any(term in value_lower for term in ["bachelor", "bsc", "ba", "beng", "undergraduate", "ug"]):
        return "Undergraduate"
    elif any(term in value_lower for term in ["phd", "doctoral", "doctorate", "dphil", "research degree"]):
        return "PhD"
    elif any(term in value_lower for term in ["master", "msc", "ma", "mba", "meng", "mphil"]):
        return "Masters"
...
def normalize_program_category(value: str) -> str:
    ...
    category_map = {
        "Computer Science": ["computer", "computing", "informatics", "software", "ai", "machine learning", "data science", "cybersecurity", "it"],
        "Networking": ["network", "telecommunication", "wireless"],
        "Business": ["business", "mba", "management", "finance", "accounting", "marketing", "economics", "commerce"],
        "Medical": ["medicine", "medical", "healthcare", "nursing", "pharmacy", "dentistry", "health", "clinical", "biomedical"],
        ...
        "Arts & Humanities": ["arts", "humanities", "literature", "history", "philosophy", "linguistics", "music"],
        "Sciences": ["biology", "chemistry", "physics", "mathematics", "statistics", "environmental"],
    }
...
class University(BaseModel):
    ...
    university_id: Optional[str] = Field(None, description="Unique identifier for the university (12-char MD5 hash)")
    name: str = Field(..., description="Official full name of the university")
    alternate_names: List[str] = Field(default_factory=list, description="Alternative names, abbreviations, or former names")
    motto: Optional[str] = Field(None, description="University motto or slogan")
    description: Optional[str] = Field(None, description="Brief description of the university")
    ...
    campus_type: Optional[str] = Field(None, description="Type of campus (urban, suburban, rural)")
    campus_size_acres: Optional[int] = Field(None, ge=0, description="Campus size in acres")

    # Contact Information
    website: Optional[str] = Field(None, description="Official university website URL")
    admissions_url: Optional[str] = Field(None, description="Admissions office website URL")
    ...
    social_media: Dict[str, str] = Field(default_factory=dict, description="Social media links (facebook, twitter, linkedin, etc.)")

    # Rankings and Reputation
    qs_world_ranking: Optional[int] = Field(None, ge=1, description="QS World University Rankings position")
    the_world_ranking: Optional[int] = Field(None, ge=1, description="Times Higher Education World Rankings position")
    us_news_ranking: Optional[int] = Field(None, ge=1, description="US News Global Universities Rankings position")
    arwu_ranking: Optional[int] = Field(None, ge=1, description="Academic Ranking of World Universities position")
    subject_rankings: Dict[str, int] = Field(default_factory=dict, description="Subject-specific rankings by field")

    # Classification and Type
    tier: Optional[str] = Field(None, description="University tier (top, good, standard)")
    type: Optional[str] = Field(None, description="University type (public, private, etc.)")
    founding_year: Optional[int] = Field(None, ge=1000, le=2100, description="Year the university was founded")
    research_intensity: Optional[str] = Field(None, description="Research intensity level (very high, high, medium, low)")

    # Size and Demographics
    total_students: Optional[int] = Field(None, ge=0, description="Total number of enrolled students")
    international_students: Optional[int] = Field(None, ge=0, description="Number of international students")
    faculty_count: Optional[int] = Field(None, ge=0, description="Total number of faculty members")
    student_faculty_ratio: Optional[float] = Field(None, gt=0, description="Student to faculty ratio")

    # Financial Information
    endowment_usd: Optional[int] = Field(None, ge=0, description="Endowment value in USD")
    average_tuition_domestic: Optional[int] = Field(None, ge=0, description="Average domestic tuition per year in USD")
    average_tuition_international: Optional[int] = Field(None, ge=0, description="Average international tuition per year in USD")

    # Facilities and Resources
    libraries_count: Optional[int] = Field(None, ge=0, description="Number of libraries")
    research_centers: List[str] = Field(default_factory=list, description="Research centers and institutes")
    sports_facilities: List[str] = Field(default_factory=list, description="Sports and recreational facilities")
    housing_options: List[str] = Field(default_factory=list, description="Available housing options")

    # Accreditation and Memberships
    accreditations: List[str] = Field(default_factory=list, description="Accreditation bodies")
    memberships: List[str] = Field(default_factory=list, description="Professional memberships and associations")

    # Student Life and Support
    student_organizations: List[str] = Field(default_factory=list, description="Student organizations and clubs")
    support_services: List[str] = Field(default_factory=list, description="Student support services")
    international_support: List[str] = Field(default_factory=list, description="International student support services")
    ...
    @field_validator("research_centers", "sports_facilities", "housing_options",
                     "accreditations", "memberships", "student_organizations",
                     "support_services", "international_support", "colors", mode="before")
    @classmethod
...
class UniversityProgram(BaseModel):
    ...
    university_id: Optional[str] = Field(None, description="Foreign key reference to University.university_id")
    ...
    degree_level: str = Field("Masters", description="Level of degree (Undergraduate/Graduate/Masters/PhD)")
    program_category: str = Field("Other", description="Program category/field (Computer Science, Business, Medical, etc.)")
    ...
    program_description: Optional[str] = Field("", description="Detailed program description")
    ...
    @field_validator("prerequisites", "specializations", "scholarships_available", 
                     "career_outcomes", "research_opportunities", "campus_facilities", mode="before")
    @classmethod
    def ensure_list(cls, v):
        """Ensure list fields are lists."""
        if v is None:
    ...
    def validate_university_id(cls, v):
        ...
        if not re.match(r'^[a-f0-9]{12}$', v):
            raise ValueError("University ID must be a valid 12-character hexadecimal string")
        return v
