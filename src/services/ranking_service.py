...
class RankingService:
    ...
    async def fetch_subject_rankings(
        ...
        try:
            async with self.scraper:
                rankings = await self.scraper.scrape_subject_rankings(subject, year, max_results)
    ...
    async def get_university_ranking(
        ...
        for ranking in rankings:
            normalized_rank_name = self._normalize_name(ranking.get('university_name', ''))
            if normalized_search in normalized_rank_name or normalized_rank_name in normalized_search:
                return ranking
    ...
    async def update_university_rankings(
        ...
        try:
            ...
            for rank in rankings:
                if self._normalize_name(rank.get('university_name', '')) == normalized_name:
                    ranking_data = rank
                    break
            ...
            if not ranking_data and university.alternate_names:
                for alt_name in university.alternate_names:
                    ...
                    for rank in rankings:
                        if self._normalize_name(rank.get('university_name', '')) == normalized_alt:
                            ranking_data = rank
                            break
                    if ranking_data:
                        break
            ...
            if ranking_data:
                ...
                logger.info(f"Updated {university.name} with rank {ranking_data.get('rank')}")
            else:
