"""University Validation and Milestone Tracker - Systematically check each university and track progress."""
...
class ValidationMilestone:
    ...
    def add_check(self, check_name: str, passed: bool, details: str = None, severity: str = "info"):
...
class UniversityValidator:
    ...
    def __init__(self):
        ...
        self.programs_data = []
        self.milestones = {}
        self.progress_file = Path(self.settings.exports_dir) / "validation_milestones.json"
            ...
            try:
                with open(self.progress_file, 'r', encoding='utf-8') as f:
                    ...
                    for uni_name, milestone_data in data.items():
                        milestone = ValidationMilestone(uni_name, milestone_data.get('university_id'))
                        milestone.checks = milestone_data.get('checks', {})
                        milestone.issues = milestone_data.get('issues', [])
                        milestone.recommendations = milestone_data.get('recommendations', [])
                ...
                console.print(f"[yellow]⚠[/yellow] Failed to load existing milestones: {e}")
    ...
    def save_milestones(self):
        ...
        try:
            ...
            data = {name: milestone.to_dict() for name, milestone in self.milestones.items()}

            with open(self.progress_file, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)

            console.print(f"[green]✓[/green] Saved {len(self.milestones)} milestones to {self.progress_file}")
        except Exception as e:
    ...
    def validate_university_basic_info(self, university: Dict) -> ValidationMilestone:
        ...
        if uni_id:
            ...
            milestone.add_check("university_id_missing", False, "University ID is missing", "warning")
        ...
        if website:
            try:
                ...
                milestone.add_check("website_url_valid", False, "Website URL format is invalid", "warning")
        else:
            milestone.add_check("website_url_missing", False, "Website URL is missing", "info")
    ...
    def validate_university_rankings(self, university: Dict) -> None:
        ...
        if tier:
            ...
            milestone.add_check("tier_missing", False, "University tier is not classified", "info")
    ...
    def validate_university_programs(self, university: Dict) -> None:
        ...
        uni_programs = [p for p in self.programs_data if p.get('university_name') == name]

        if not uni_programs:
            milestone.add_check("no_programs", False, "No programs found for this university", "error")
        ...
        programs_with_descriptions = sum(1 for p in uni_programs if p.get('program_description', '').strip())
        programs_with_fees = sum(1 for p in uni_programs if p.get('tuition_fees', {}).get('domestic_per_year'))

        description_coverage = programs_with_descriptions / len(uni_programs) if uni_programs else 0
        ...
        program_names = [p.get('program_name', '').lower().strip() for p in uni_programs]
    ...
    def validate_university_completeness(self, university: Dict) -> None:
        ...
        filled_fields = sum(1 for field in important_fields if university.get(field) is not None)
        ...
        if completeness < 0.6:
            milestone.add_recommendation("Improve data completeness by adding missing fields")
    ...
    async def validate_university_website(self, university: Dict) -> None:
        ...
        if not website:
            milestone.add_check("website_accessibility", False, "No website URL to test", "info")
            return
    ...
    async def validate_all_universities(self, check_websites: bool = False) -> Dict[str, ValidationMilestone]:
        ...
        with Progress(
            ...
            main_task = progress.add_task("Validating universities...", total=len(self.universities_data))
    ...
    def generate_validation_report(self) -> Dict[str, Any]:
        ...
        score_categories = {
            ...
            "needs_improvement": sum(1 for m in self.milestones.values() if 0.5 <= m.score < 0.7),
