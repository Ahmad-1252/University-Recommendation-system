...
class TopUniversitiesEnricher:
    ...
    async def enrich_university(self, university: University) -> EnrichmentResult:
        ...
        try:
            ...
            if updated_university.qs_world_ranking != result.before_rank:
                ...
                console.print(f"  [green]✓ Ranking: {result.before_rank or 'None'} → {result.after_rank}[/green]")
            
            if updated_university.country != before_country:
                result.fields_updated.append("country")
                console.print(f"  [green]✓ Country: {before_country or 'None'} → {updated_university.country}[/green]")
            
            if updated_university.city != before_city:
                result.fields_updated.append("city")
                console.print(f"  [green]✓ City: {before_city or 'None'} → {updated_university.city}[/green]")
            ...
            if result.fields_updated:
                ...
                result.success = True
                console.print(f"  [bold green]✓ Saved {len(result.fields_updated)} updates[/bold green]")
            else:
                result.success = True
    ...
    async def enrich_all(self, university_filter: str = None) -> Dict[str, EnrichmentResult]:
        ...
        try:
            if university_filter:
                universities = await self.university_repo.find_by_name(university_filter)
                if not universities:
                    # Try partial match
                    all_unis = await self.university_repo.get_all()
                    universities = [u for u in all_unis if university_filter.lower() in u.name.lower()]
            else:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        no_changes = sum(1 for r in self.results.values() if r.success and not r.fields_updated)
        
        total_fields_updated = sum(len(r.fields_updated) for r in self.results.values())
        
        # Count ranking updates
        ranking_updates = [r for r in self.results.values() if "qs_world_ranking" in r.fields_updated]
...
Examples:
    ...
    try:
        async with TopUniversitiesEnricher(force_refresh=args.force_refresh) as enricher:
