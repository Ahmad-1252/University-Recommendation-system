...
def map_research_intensity(research: str) -> Optional[str]:
    ...
    mapping = {"VH": "very high", "H": "high", "M": "medium", "L": "low"}
...
def import_rankings(
    file_path: str, dry_run: bool = False, limit: Optional[int] = None, year: int = 2026
) -> Dict[str, int]:
    """
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of records to import"
    )
    parser.add_argument(
        "--year", type=int, default=2026, help="Ranking year (default: 2026)"
    )
    ...
    stats = import_rankings(
        file_path=args.file, dry_run=args.dry_run, limit=args.limit, year=args.year
    )
    ...
    if stats["errors"] > 0:
        console.print(
            f"[yellow]Warning: {stats['errors']} errors occurred during import[/yellow]"
        )
    else:
