...
class DataExporter:
    ...
    def show_data_summary(self):
        ...
        if self.programs_data:
            ...
            universities = set(
                p.get("university_name", "Unknown") for p in self.programs_data
            )
            degree_types = set(
                p.get("degree_type", "Unknown") for p in self.programs_data
            )
        ...
        if self.universities_data:
            ...
            tiers = set(
                u.get("tier", "Unknown")
                for u in self.universities_data
                if u.get("tier")
            )
    ...
    def choose_export_format(self) -> str:
        ...
        for i, fmt in enumerate(formats, 1):
            if fmt == "csv":
                console.print(
                    f"  [{i}] CSV - Comma-separated values (compatible with Excel)"
                )
            elif fmt == "json":
                console.print(
                    f"  [{i}] JSON - JavaScript Object Notation (structured data)"
                )
            elif fmt == "xlsx":
        ...
        while True:
            try:
                choice = Prompt.ask(
                    "Enter your choice",
                    choices=[str(i) for i in range(1, len(formats) + 1)],
                )
    ...
    def export_csv(
        self, data: List[Dict], filename: str, data_type: str = "unknown"
    ) -> bool:
        ...
        try:
            ...
            console.print(
                f"[green]✓[/green] Exported {len(data)} records to {filepath}"
            )
            return True
    ...
    def export_excel(
        ...
        try:
            ...
            if PANDAS_AVAILABLE:
                ...
                console.print(
                    "[red]✗[/red] Neither pandas nor openpyxl available for Excel export"
                )
                return False
    ...
    def _export_excel_pandas(
        self, data: List[Dict], filepath: Path, data_type: str = "unknown"
    ) -> bool:
        """Export using pandas."""
        try:
            ...
            console.print(
                f"[green]✓[/green] Exported {len(data)} records to {filepath} (pandas)"
            )
            return True
    ...
    def _export_excel_openpyxl(
        ...
        try:
            ...
            wb.save(filepath)
            console.print(
                f"[green]✓[/green] Exported {len(data)} records to {filepath} (openpyxl)"
            )
            return True
