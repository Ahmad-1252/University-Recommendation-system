...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/url_validation.log')
    ]
)
    ...
    try:
        ...
        if valid_urls:
            ...
            for url, reason in valid_urls:
                university = next((k for k, v in UNIVERSITY_URLS.items() if v == url), "Unknown")
        ...
        validation_results = {
            ...
            "valid_urls": [{"university": next((k for k, v in UNIVERSITY_URLS.items() if v == url), "Unknown"),
                           "url": url, "reason": reason} for url, reason in valid_urls],
            "invalid_urls": [{"university": next((k for k, v in UNIVERSITY_URLS.items() if v == url), "Unknown"),
                            "url": url, "reason": reason} for url, reason in invalid_urls]
        }
