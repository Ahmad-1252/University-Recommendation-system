...
def verify_database_content():
    ...
    try:
        ...
        logger.info(
            f"Connecting to Database: '{DATABASE_NAME}', Collection: '{COLLECTION_NAME}'"
        )
        ...
        if total_records > 0:
            ...
            print(
                f"Please check the '{DATABASE_NAME}' database and '{COLLECTION_NAME}' collection in your MongoDB client."
            )
            ...
            logger.error(
                "This indicates a problem with the scraper's saving mechanism or the database connection."
            )
            print("\n" + "=" * 80)
        ...
        logger.error("❌ DATABASE CONNECTION FAILED: Could not connect to MongoDB.")
        logger.error(
            f"Please ensure MongoDB is running at the URI specified in your .env file: {os.getenv('MONGODB_URI')}"
        )
