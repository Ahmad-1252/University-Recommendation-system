...
class BottomUpUniversityTester:
    ...
    def _get_directory_levels(self, config: Dict[str, Any]) -> Dict[str, str]:
        ...
        for key in level_keys:
            if key in config and key not in [
                "base_url",
                "program_url_pattern",
                "pagination",
            ]:
    ...
    async def test_university(
        self, name: str, config: Dict[str, Any]
    ) -> UniversityTestResult:
        ...
        if not directory_levels:
            console.print("  [red]✗ No directory URLs found in config[/red]")
            result.status = "failed"
            return result

        console.print(f"  [dim]Base URL: {base_url}[/dim]")
        console.print(
            f"  [dim]Pattern: {program_url_pattern or 'None (heuristic mode)'}[/dim]"
        )
        console.print(
            f"  [dim]Levels to test: {', '.join(directory_levels.keys())}[/dim]"
        )
        ...
        console.print(
            f"  [dim]Programs per level: ~{programs_per_level} (total target: {self.total_programs})[/dim]"
        )
        ...
        for level_name, directory_url in directory_levels.items():
            ...
            try:
                ...
                console.print(
                    f"     [green]✓ Discovered: {len(links)} program links[/green]"
                )
                ...
                if not self.dry_run and links:
                    ...
                    console.print(
                        f"     [yellow]Scraping {len(programs_to_scrape)} programs...[/yellow]"
                    )
                    ...
                    for link in programs_to_scrape:
                        try:
                            ...
                            if url:
                                program = await self.scraper.scrape_program_data(
                                ...
                                if program:
                                    ...
                                    console.print(
                                        f"       [green]✓ {program.program_name[:50]}[/green]"
                                    )
                                else:
                                    console.print(
                                        "       [red]✗ Failed to extract[/red]"
                                    )
        ...
        console.print(
            f"  Total Links: {total_links} | Scraped: {total_scraped} | Duration: {result.duration_seconds:.1f}s"
        )
    ...
    def print_summary(self):
        ...
        for idx, name in enumerate(self.UNIVERSITY_ORDER, 1):
            ...
            levels_ok = sum(
                1 for lr in result.level_results.values() if lr.status == "success"
            )
        ...
        for level_name, stats in sorted(level_stats.items()):
            success_rate = (
                (stats["success"] / stats["tested"] * 100) if stats["tested"] > 0 else 0
            )
            level_table.add_row(
                level_name,
                str(stats["tested"]),
        ...
        failed_universities = [
            (n, r) for n, r in self.results.items() if r.status == "failed"
        ]
        if failed_universities:
            ...
            for name, result in failed_universities:
                ...
                for level_name, lr in result.level_results.items():
                    if lr.status == "failed":
                        console.print(
                            f"    [dim]{level_name}: {lr.error or 'No links found'}[/dim]"
                        )
...
Examples:
    ...
    parser.add_argument(
        "--limit", type=int, help="Limit number of universities to test (from bottom)"
    )
    parser.add_argument(
        "--skip",
        type=int,
    ...
    parser.add_argument("--output", type=str, help="Save detailed report to JSON file")

    args = parser.parse_args()

    tester = BottomUpUniversityTester(
        total_programs=args.programs, dry_run=args.dry_run
    )
