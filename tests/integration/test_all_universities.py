...
class UniversityTester:
    ...
    async def test_university(
        ...
        for level in directory_levels:
            if level in config and level not in [
                "base_url",
                "program_url_pattern",
                "pagination",
            ]:
        ...
        console.print(
            f"  [dim]Directory URLs: {len(result.directory_urls_tested)}[/dim]"
        )
        console.print(
        ...
        for directory_url in result.directory_urls_tested[
            :2
        ]:  # Test first 2 levels max
            try:
                ...
                if links:
                    ...
                    for link in links[:3]:
                        console.print(
                            f"    [dim]- {link.get('program_name', 'Unknown')[:50]}[/dim]"
                        )
                        console.print(
                            f"      [dim]{link.get('program_url', '')[:80]}[/dim]"
                        )
                        ...
                        try:
                            ...
                            if url:
                                ...
                                if program:
                                    ...
                                    console.print(
                                        f"    [green]✓ {program.program_name}[/green]"
                                    )
                                else:
                                    console.print(
                                        f"    [red]✗ Failed to extract: {url[:60]}[/red]"
                                    )
    ...
    async def test_all(
        self, university_filter: Optional[str] = None
        ...
        if not universities_to_test:
            console.print(
                f"[red]No universities found matching filter: {university_filter}[/red]"
            )
            return {}
...
Examples:
    ...
    tester = UniversityTester(max_programs=args.max_programs, dry_run=args.dry_run)
