...
def main():
    ...
    try:
        ...
        try:
            ...
            pandas_result = exporter.export_to_pandas_excel(
                programs, "universities_programs_pandas.xlsx"
            )
            print(
                "    Status: [OK] SUCCESS\n"
                if pandas_result
        ...
        if programs:
            ...
            if confidences:
                ...
                print(
                    f"   - Average Confidence: {avg_confidence:.2f} ({avg_confidence*100:.0f}%)"
                )

            # Degree types
            degrees = set(
                p.get("degree_type", "Unknown")
                for p in programs
