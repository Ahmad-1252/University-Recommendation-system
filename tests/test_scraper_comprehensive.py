...
class ScraperTestSuite:
    ...
    def test_mongodb_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[PASS] Connection successful (Duration: {duration*1000:.2f}ms)"
            )
            return result

        except Exception as e:
            result = {"test": "MongoDB Connection", "status": "FAIL", "error": str(e)}
    ...
    def test_database_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"test": "Database Integrity", "status": "FAIL", "error": str(e)}
            logger.error(f"[FAIL] {e}")
            return result
    ...
    def test_data_schema_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if invalid_programs:
                logger.warning(
                    f"[WARNING] {len(invalid_programs)} programs with missing fields"
                )
                for prog in invalid_programs[:3]:  # Show first 3
                    logger.warning(
                        f"  - {prog['program_name']}: Missing {prog['missing_fields']}"
                    )
        ...
        try:
            ...
            for uni in expected_universities:
                ...
                coverage_data.append(
                    {"university": uni, "program_count": count, "status": status}
                )
                total_programs += count
                logger.info(f"[{status}] {uni}: {count} programs")

            universities_with_data = sum(
                1 for c in coverage_data if c["program_count"] > 0
            )
            coverage_percentage = (
                universities_with_data / len(expected_universities)
            ) * 100
    ...
    def test_data_quality_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if confidence_scores:
                metrics["avg_confidence_score"] = sum(confidence_scores) / len(
                    confidence_scores
                )
            ...
            logger.info(
                f"[OK] Average confidence score: {metrics['avg_confidence_score']:.2f}"
            )
            logger.info(
                f"[OK] Programs with description: {metrics['programs_with_description']}/{metrics['total_analyzed']}"
            )
            logger.info(
                f"[OK] Programs with duration: {metrics['programs_with_duration']}/{metrics['total_analyzed']}"
            )
            logger.info(
                f"[OK] Programs with requirements: {metrics['programs_with_requirements']}/{metrics['total_analyzed']}"
            )
            logger.info(
                f"[OK] Programs with fees: {metrics['programs_with_fees']}/{metrics['total_analyzed']}"
            )
            logger.info(
                f"[OK] Programs with deadline: {metrics['programs_with_deadline']}/{metrics['total_analyzed']}"
            )
            ...
            field_coverage_score = (
                ...
                + (metrics["programs_with_requirements"] / metrics["total_analyzed"])
                * 20
                + (metrics["programs_with_fees"] / metrics["total_analyzed"]) * 20
                + (metrics["programs_with_deadline"] / metrics["total_analyzed"]) * 20
            )

            overall_quality = (metrics["avg_confidence_score"] * 50) + (
                field_coverage_score / 5 * 50
            )
            ...
            result = {"test": "Data Quality Metrics", "status": "FAIL", "error": str(e)}
            logger.error(f"[FAIL] {e}")
            return result
    ...
    def test_groq_api_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[{"role": "user", "content": "Say 'API working' only"}],
                max_tokens=20,
            )
            duration = time.time() - start
            ...
            result = {"test": "Groq API Integration", "status": "FAIL", "error": str(e)}
            logger.error(f"[FAIL] {e}")
            return result
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        warnings = sum(
            1 for t in self.test_results["tests"] if t.get("status") == "WARNING"
        )
